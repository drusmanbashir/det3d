...
def to_dtype(inp, dtype: Callable):
    if isinstance(inp, Sequence) and not isinstance(inp, (str, bytes)):
        return type(inp)([to_dtype(i, dtype) for i in inp])
    if isinstance(inp, torch.Tensor):
        return inp.to(dtype=dtype)
    return dtype(inp)
