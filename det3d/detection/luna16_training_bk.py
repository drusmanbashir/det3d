...
from det3d.managers.data import bbv
...
# %%
    ...
    # d1 = raw datalist dict; dA = A(d1); ...; dM = M(dL) list[dict]; dN = N(dM[0]) ...
# %%
    d1["box"]
    dA = A(d1)
    dB = B(dA)
...
# %%
    ...
    print(dS["points"])
# %%
# one corner: nodule 0, corner 0

    p = dJ["points"][0, 0]; p                    # J: world mm (same as dI box corners)
    p = dK["points"][0, 0]; p                    # K: unchanged — K only moved box
    p = dR["points"][0, 0]; p                    # M–R: still world mm
    print(dR["image"].meta["affine"])
    A = dR["image"].meta["affine"]
    torch.round(A)
    (torch.linalg.inv(A) @ torch.tensor([*p, 1.0], dtype=A.dtype))[:3]  # manual world→patch voxel
    p = dS["points"][0, 0]; p                    # S: patch voxel (should match ↑)
    p = dT["points"][0, 0]; p                    # T: same coords, now also in dT["box"][0]
    p = dU["points"][0, 0]; p; dU["box"][0]      # U: clip box; points unchanged
# %%
    dT = T(dS)
    dU = U(dT)
    dV = V(dU)
...
# %%
    ...
    # for bb2 in bb:
    bb2 = bb[2]
    print (bb2['image'].shape)
    print(bb2['box'])
    im = bb2['image'][0]
    bbox = bb2['box']
    ImageBBoxViewer(im, bbox)
