...
if _repo_root_s in sys.path:
    sys.path.remove(_repo_root_s)
    sys.path.append(_repo_root_s)
...
#SECTION:-------------------- setup --------------------------------------------------------------------------------------
    ...
    from lidc_datalist import load_lidc_train_val
    from monai.transforms import ScaleIntensityRanged

    train_data, val_data, meta = load_lidc_train_val("lidc", plan_id=1, ds_name="lidc")
    print(
        f"train cases {meta['train_cases']} val cases {meta['val_cases']} "
        f"errors {meta['errors']}"
    )

    clip = meta["dataset_params"]["intensity_clip_range"]
    ...
    config_path = Path(__file__).resolve().parent / "config" / "config_train_luna16_16g.json"
    with config_path.open() as config_file:
        train_config = json.load(config_file)
    patch_size = train_config["patch_size"]
    batch_size = int(train_config["batch_size"])
    gt_box_mode = train_config["gt_box_mode"]
    train_tfms = generate_detection_train_transform(
        "image",
        "box",
        "label",
        gt_box_mode,
    ...
    val_tfms = generate_detection_val_transform(
        "image",
        "box",
        "label",
        gt_box_mode,
    ...
    sample = train_tfms(train_data[0])
    print(f"train_tfms output n={len(sample)} image shape {sample[0]['image'].shape}")
    print(f"boxes {sample[0]['box'].shape} labels {sample[0]['label'].shape}")
